...
const pathCheck = ["/reset-password", "/register", "/forgot-password"];
...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (new Date().getTime() < new Date(2023, 6, 30).getTime()) {
      setShowModal(true);
    }
  }, []);
  ...
  const onSubmit = useCallback(
    async (values: UserInputTypeLogi) => {
      ...
      try {
        ...
        const check = pathCheck.findIndex((p) => prevPath.startsWith(p));
